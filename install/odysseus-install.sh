...
# Global helper functions to ensure they exist regardless of how the script is sourced
STD() { "$@" >/dev/null 2>&1; }
msg_info() { echo -e "\033[0;34m[INFO]\033[0m $1"; }
msg_ok() { echo -e "\033[0;32m[OK]\033[0m $1"; }
msg_error() { echo -e "\033[0;31m[ERROR]\033[0m $1"; }
...
# Redefine helpers to ensure they override any community funcs that might have cleared them
STD() { "$@" >/dev/null 2>&1; }
msg_info() { echo -e "\033[0;34m[INFO]\033[0m $1"; }
msg_ok() { echo -e "\033[0;32m[OK]\033[0m $1"; }
msg_error() { echo -e "\033[0;31m[ERROR]\033[0m $1"; }

# --- Local overrides for standalone mode ---
...
cd /opt
STD curl -fsSL https://github.com/pewdiepie-archdaemon/odysseus/archive/refs/heads/main.tar.gz | tar -xz --strip-components=1 -C /opt/odysseus
STD /opt/odysseus/.venv/bin/pip install -r /opt/odysseus/requirements.txt
systemctl restart odysseus
msg_ok "Odysseus updated successfully!"
UPDATE_SCRIPT
...
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/root/.local/bin:$PATH"
msg_info "Downloading Odysseus Archive"
mkdir -p /opt/odysseus
STD curl -fsSL --retry 3 --retry-delay 5 -o /opt/odysseus/odysseus.tar.gz https://github.com/pewdiepie-archdaemon/odysseus/archive/refs/heads/main.tar.gz
STD tar -xzf /opt/odysseus/odysseus.tar.gz --strip-components=1 -C /opt/odysseus
STD rm -f /opt/odysseus/odysseus.tar.gz
msg_ok "Downloaded and Extracted Odysseus"
...
cd /opt/odysseus
ODYSSEUS_ADMIN_PASSWORD="$ADMIN_PASS" timeout 120 /opt/odysseus/.venv/bin/python3 setup.py
